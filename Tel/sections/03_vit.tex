...
\begin{tabular}{rll}
\toprule
 & \textbf{Przykład} & \textbf{ViT-Small} \\
\midrule
...
\textit{Wszystkie patche} & -- & $\approx\mathbf{57.8\text{M}}$ operacji \\
\bottomrule
\end{tabular}}
\end{center}
...
& $\mathbb{R}^{384}$ \\
wejście & $4\times$ & ukryta & $/4$ & wyjście
\end{tabular}
\end{center}
...
\midrule
1 & Przed Self-Att. & Stabilizuje $QK^\top$ \\
2 & Przed MLP & Stabilizuje GELU \\
\bottomrule
\end{tabular}
\end{center}
...
Gradient & przechodzi przez LN & \textbf{skrót rezydualny} \\
...
\midrule
Normalizuje & po próbkach & po wymiarach \\
Statystyki & z batcha ($B$) & z tokenu ($d$) \\
...
$\mathbf{h}^{(0)}$ & $\xrightarrow{\text{auto}}$ & $\mathbf{h}^{(1)}$
& $\xrightarrow{\text{auto}}$ & $\cdots$
& $\xrightarrow{\text{auto}}$ & $\mathbf{h}^{(12)}$ \\[4pt]
& $\uparrow$ \small{+$\Delta_1$} & & $\uparrow$ \small{+$\Delta_2$} & & $\uparrow$ \small{+$\Delta_{12}$} & \\[2pt]
& \fbox{\scriptsize Blok 1} & & \fbox{\scriptsize Blok 2} & & \fbox{\scriptsize Blok 12} & \\
\end{tabular}}
\end{center}
