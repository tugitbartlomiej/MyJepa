...
\begin{tabular}{p{1.5cm}p{0.65\textwidth}}
\toprule
...
\begin{center}
\renewcommand{\arraystretch}{1.6}
\begin{tabular}{lcc p{0.4\textwidth}}
\toprule
...
\begin{multline}
\mathcal{L}[p] = \underbrace{-\!\int\! p\ln p\,dz}_{\text{entropia}}
- \lambda_0\!\underbrace{\left(\!\int\! p\,dz - 1\!\right)}_{\text{norm.}} \\
...
\begin{align}
\text{(I)}\; \frac{\partial}{\partial p}\big[-p\ln p\big] &= -\ln p - 1
  \label{eq:var_I}\\
\text{(II)}\; \frac{\partial}{\partial p}\big[-\lambda_0 p\big] &= -\lambda_0
  \label{eq:var_II}\\
\text{(III)}\; \frac{\partial}{\partial p}\big[-\lambda_1 z\,p\big] &= -\lambda_1 z
  \label{eq:var_III}\\
\text{(IV)}\; \frac{\partial}{\partial p}\big[-\lambda_2(z{-}\mu)^2 p\big] &= -\lambda_2(z{-}\mu)^2
  \label{eq:var_IV}
\end{align}
...
\begin{tabular}{lll}
\toprule
\textbf{Ograniczenie} & \textbf{Lagrangian} & \textbf{Wynik} \\
\midrule
...
Wyznaczamy $\alpha$ z~warunku $\operatorname{Var}[Z] = \sigma^2$.

Bez straty ogólności centrujemy: $\mu = 0$. Potrzebujemy:
\[
\sigma^2 = \int_{-\infty}^{\infty}\! z^2\,\tilde{p}(z)\,dz,
\quad \tilde{p}(z) = \frac{e^{-\alpha z^2}}{\int_{-\infty}^{\infty}\! e^{-\alpha t^2}dt}
\]
...
Wariancja to stosunek dwóch całek (licznik / mianownik):
\[
\sigma^2
= \frac{\int z^2\,e^{-\alpha z^2}\,dz}{\int e^{-\alpha z^2}\,dz}
= \frac{\tfrac{1}{2}\sqrt{\pi/\alpha^3}}{\sqrt{\pi/\alpha}}
\]

Upraszczamy --- $\sqrt{\pi}$ skraca się:
\[
\sigma^2
= \frac{\tfrac{1}{2}\,\alpha^{-3/2}}{\alpha^{-1/2}}
= \frac{1}{2}\cdot\alpha^{-3/2}\cdot\alpha^{1/2}
= \frac{1}{2}\cdot\alpha^{-1}
= \frac{1}{2\alpha}
\]
...
\det(\boldsymbol{\Sigma}) = \sigma_1^2 \sigma_2^2
\end{equation}
...
\begin{remark}
Gdy $\rho = 0$: $\frac{1}{1-\rho^2} = 1$ i wyraz mieszany znika -- przypadek niezależny.
\end{remark}
